...
const VERTICAL_LEGENDS: Record<Vertical, { name: string; result: string; context: string }> = {
  "Enterprise SaaS": { name: "Slack", result: "$100M in 30mo", context: "The PLG-to-Enterprise standard." },
  "Fintech": { name: "Ramp", result: "$100M in 24mo", context: "Mastery of fintech unit economics." },
  "AI Infrastructure": { name: "Cursor", result: "$100M in 12mo", context: "The new AI-native hypergrowth record." },
  "Cybersecurity": { name: "Wiz", result: "$100M in 18mo", context: "Fastest security scale-up in history." },
  "DevTools": { name: "Semgrep", result: "5x YoY Growth", context: "Nav architected this revenue engine." },
};
...
export default function VelocityCalculator() {
  ...
  const legend = VERTICAL_LEGENDS[inputs.vertical];
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(Object.keys(VERTICAL_LEGENDS) as Vertical[]).map((v) => (
                      <button
              ...
              >
                Benchmark against {inputs.vertical} Legends
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-muted uppercase tracking-widest">Industry Legend</span>
                    <span className="text-emerald-400 font-black text-xl">{legend.name}</span>
                  </div>
                  <div className="text-2xl font-bold text-white">{legend.result}</div>
                  <p className="text-sm text-muted leading-relaxed">{legend.context}</p>
                </div>
                <div className="bg-red-400/5 border border-red-400/20 rounded-3xl p-8 space-y-4">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm text-muted leading-relaxed">This is the unrealized ARR based on top-quartile efficiency targets for {inputs.stage} companies.</p>
                </div>
              </div>

              <div className="bg-accent/5 border border-accent/20 rounded-3xl p-8 lg:p-12 text-center space-y-6">
                <h3 className="text-2xl font-bold">The 6-Month Opportunity</h3>
                <p className="text-lg text-muted max-w-2xl mx-auto leading-relaxed">
                  Based on our GTM architecture at <strong>HeyGen</strong> and <strong>Semgrep</strong>, we typically drive a <span className="text-accent font-bold">15-30% lift</span> in win rate within 6 months by implementing agentic workflows.
                </p>
                <button
                  onClick={() => setStep(3)}
              ...
              <div className="bg-white text-[#0B1221] rounded-[3rem] p-10 lg:p-16 space-y-8 shadow-[0_0_100px_rgba(255,255,255,0.1)] relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-8">
                  ...
                  <p className="text-lg font-medium max-w-2xl mx-auto opacity-80 leading-relaxed">
                    Schedule a 15-minute strategy audit. We&apos;ll walk through your specific bottlenecks and share the exact agentic workflows we used to scale HeyGen to $100M ARR.
                  </p>
